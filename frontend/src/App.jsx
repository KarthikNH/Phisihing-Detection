...
const SAMPLE_URLS = [
  { label: 'Google (Verified Safe)', url: 'https://www.google.com' },
  { label: 'Linux GitHub (Verified Safe)', url: 'https://github.com/torvalds/linux' },
  { label: 'PayPal Spoof (Phishing)', url: 'http://paypal-security-update.xyz/login?id=99283' },
  { label: 'IP Host Admin (Suspicious)', url: 'http://192.168.1.1/admin/login' }
];
...
export default function App() {
  ...
  const [chatMessages, setChatMessages] = useState([
    { 
      sender: 'ai', 
      text: "Greetings. I am PHISHGUARD AI, your real-time threat intelligence analyst. Enter a URL above to inspect machine learning telemetry, or select a prompt below." 
    }
  ]);
  const [chatInput, setChatInput] = useState('');
  const [chatLoading, setChatLoading] = useState(false);

  // API helper that seamlessly works in both Vite dev server and direct production serving
  const fetchWithFallback = async (endpoint, options = {}) => {
    try {
      // First attempt relative request (handled by Vite dev proxy or FastAPI production server)
      const res = await fetch(endpoint, options);
      if (res.ok || res.status === 400 || res.status === 404 || res.status === 405) {
        return res;
      }
    } catch {
      // Fallback directly to localhost:8000 in case proxy or relative host is disconnected
    }
  ...
  const loadMetrics = async () => {
    try {
      const res = await fetchWithFallback('/metrics');
      if (res.ok) {
        const data = await res.json();
        setMetricsData(data);
      }
    } catch (e) {
      console.warn('Backend metrics not yet loaded:', e);
    }
  };

  const handleAnalyze = async (targetUrl = urlInput) => {
    const rawUrl = (targetUrl !== undefined && targetUrl !== null) ? targetUrl : urlInput;
    ...
    try {
      const res = await fetchWithFallback('/analyze', {
        method: 'POST',
      ...
      if (!res.ok) {
        let errDetail = 'Security engine could not be reached. Check that the local API is running.';
        try {
          ...
          if (res.status === 405) {
            errDetail = 'ANALYSIS UNAVAILABLE: Method not allowed on analysis route. Ensure API is running.';
          }
        }
      ...
      setChatMessages(prev => [
        ...
        {
          sender: 'ai',
          text: `Telemetry synchronized for \`${data.url}\`.\n\n• Risk Level: **${data.risk_level}**\n• Composite Index: **${data.risk_score}/100**\n• Phishing Probability: **${(data.phishing_probability * 100).toFixed(1)}%**\n• Anomaly Index: **${data.anomaly_score.toFixed(2)}**\n\nAsk me why this was flagged or what actions you should take.`
        }
      ]);
    } catch (err) {
      console.error('Analysis error:', err);
      setError(err.message || 'ANALYSIS UNAVAILABLE: Security engine could not be reached. Check that the local API is running.');
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (res.ok) {
        ...
        setChatMessages(prev => [
          ...
          { 
            sender: 'ai', 
            text: 'ASSISTANT UNAVAILABLE: Unable to process chat request at this time. Verify that the local API server is active.' 
          }
        ]);
      }
    } catch (err) {
      ...
      setChatMessages(prev => [
        ...
        { 
          sender: 'ai', 
          text: 'ASSISTANT UNAVAILABLE: Could not reach PhishGuard AI service. Check that the local API is running.' 
        }
      ]);
    } finally {
    ...
    switch (level) {
      ...
      case 'HIGH':
        return {
          ...
          desc: 'High probability of phishing or credential harvesting. Do not submit passwords, payment credentials, or personal information. Close the tab immediately.',
          type: 'danger'
        };
      case 'MEDIUM':
        return {
          title: 'EVALUATE WITH CAUTION — SUSPICIOUS PATTERNS',
          desc: 'Unusual lexical features, non-standard subdomains, or high entropy detected. Verify sender and domain spelling before continuing.',
          type: 'warning'
        };
      default:
        return {
          title: 'VERIFIED LOW RISK — SAFE TO PROCEED',
          desc: 'Domain structure matches normal web conventions with negligible anomaly signals. Maintain standard web hygiene and HTTPS lock verification.',
          type: 'safe'
        };
    }
  };
    ...
    <div className="min-h-screen flex flex-col relative text-slate-100 bg-[#040508] selection:bg-cyan-500/30 selection:text-white">
      ...
      <div className="border-b border-white/[0.06] bg-[#050609]/90 backdrop-blur-md px-6 py-2.5 text-[11px] font-mono tracking-wider flex justify-between items-center text-slate-400">
        <div className="flex items-center gap-4">
          ...
          <span className="hidden sm:inline text-slate-500">ML INFERENCE ENGINE v1.0</span>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${backendHealth?.status === 'healthy' ? 'bg-emerald-400 shadow-sm shadow-emerald-400 animate-ping' : 'bg-rose-500'}`} />
            <span className={backendHealth?.status === 'healthy' ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'}>
              {backendHealth?.status === 'healthy' ? 'API RUNNING // MODEL ACTIVE' : 'API OFFLINE'}
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <nav className="flex items-center gap-1.5 bg-black/40 border border-white/[0.08] p-1 rounded-xl">
            <button 
              ...
              className={`px-4 py-1.5 rounded-lg text-xs font-mono font-medium transition-all flex items-center gap-2 ${
                activeTab === 'scanner' 
                  ? 'bg-white/[0.1] text-white border border-white/[0.12] shadow-sm' 
                  : 'text-slate-400 hover:text-white'
              }`}
            >
              <Activity className="w-3.5 h-3.5 text-cyan-400" /> THREAT SCANNER
            </button>
            <button 
            ...
            >
              <Cpu className="w-3.5 h-3.5 text-cyan-400" /> MODEL INTELLIGENCE
            </button>
          </nav>
      ...
      <main className="max-w-7xl mx-auto px-6 pt-12 pb-24 w-full flex-grow relative">
                  ...
                  <div className="cyber-panel p-6 flex flex-col items-center justify-center text-center space-y-3">
                    ...
                    <div className="relative w-36 h-36 flex items-center justify-center my-1">
                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                        <circle cx="50" cy="50" r="40" stroke="rgba(255,255,255,0.06)" strokeWidth="7" fill="transparent" />
                        <circle 
                          cx="50" cy="50" r="40" 
                          stroke={
                            analysisResult.risk_score >= 85 ? '#ff2a5f' : 
                            analysisResult.risk_score >= 60 ? '#ff6b00' : 
                            analysisResult.risk_score >= 30 ? '#eab308' : '#10b981'
                          }
                          strokeWidth="7" 
                    ...
                    <div className={`px-4 py-1 rounded-full text-[11px] font-mono font-bold tracking-widest ${getRiskBadgeClass(analysisResult.risk_level)}`}>
                  ...
                  <div className="cyber-panel p-6 flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 mt-1 font-mono">
                        Model: {analysisResult.model_used || 'XGBoost'}
                      </p>
                    </div>

                    <div className="w-full bg-white/[0.05] rounded-full h-2 overflow-hidden mt-4">
                      <div 
                        className={`h-full transition-all duration-1000 ${analysisResult.phishing_probability >= 0.5 ? 'bg-rose-500 shadow-sm shadow-rose-500' : 'bg-emerald-400 shadow-sm shadow-emerald-400'}`}
                        style={{ width: `${Math.min(100, Math.max(2, analysisResult.phishing_probability * 100))}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="cyber-panel p-6 flex flex-col justify-between">
                    ...
                    <div className="w-full bg-white/[0.05] rounded-full h-2 overflow-hidden mt-4">
                      <div 
                        ...
                        style={{ width: `${Math.min(100, Math.max(2, analysisResult.anomaly_score * 100))}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="cyber-panel p-6 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-3">
                        {String(analysisResult.prediction).toLowerCase() === 'phishing' || analysisResult.prediction_code === 1 ? (
                          <ShieldAlert className="w-9 h-9 text-rose-500 flex-shrink-0" />
                        ) : (
                          <ShieldCheck className="w-9 h-9 text-emerald-400 flex-shrink-0" />
                        )}
                        <div>
                          <span className={`editorial-title text-2xl font-bold ${
                            String(analysisResult.prediction).toLowerCase() === 'phishing' || analysisResult.prediction_code === 1 ? 'text-rose-400' : 'text-emerald-400'
                          }`}>
                            {String(analysisResult.prediction || analysisResult.prediction_label).toUpperCase()}
                          </span>
                          <p className="text-[10px] text-slate-400 font-mono">Deterministic ML Output</p>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      <MessageSquare className="w-3.5 h-3.5" /> Ask PHISHGUARD AI about this link
                    </button>
                  </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className={`p-5 rounded-2xl border flex items-start gap-4 ${
                      rec.type === 'danger' ? 'bg-rose-950/20 border-rose-500/30 text-rose-200' :
                      rec.type === 'warning' ? 'bg-amber-950/20 border-amber-500/30 text-amber-200' :
                      ...
                      <div>
                        <h4 className="font-mono font-bold text-sm tracking-wide">{rec.title}</h4>
                        <p className="text-xs mt-1 text-slate-300 font-sans leading-relaxed">{rec.desc}</p>
                      </div>
                    </div>
                  );
                })()}
                ...
                {analysisResult.features && (
                  <div className="cyber-panel p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
                      ...
                      <FeatureBox label="Subdomains" value={analysisResult.features.NoOfSubDomain} />
                      ...
                      <FeatureBox label="Entropy" value={Number(analysisResult.features.Entropy || 0).toFixed(2)} />
        ...
        {activeTab === 'intelligence' && (
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            {/* Classifier Comparison Matrix Table */}
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse text-xs font-mono">
                    ...
                    <tbody className="divide-y divide-white/[0.04]">
                      {Object.entries(metricsData.metrics).map(([name, m], idx) => (
                        <tr key={idx} className={name === metricsData.best_model ? 'bg-cyan-500/[0.06]' : ''}>
                          ...
                          <td className="p-3 text-slate-200">{(m.Precision * 100).toFixed(2)}%</td>
                          <td className="p-3 text-slate-200">{(m.Recall * 100).toFixed(2)}%</td>
                          <td className="p-3 text-cyan-400 font-bold">{(m.F1_Score * 100).toFixed(2)}%</td>
                          <td className="p-3 text-emerald-400 font-bold">{m.ROC_AUC.toFixed(4)}</td>
                          <td className="p-3 text-slate-400">ARTIFACT LOADED</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <VisualPlotCard title="Isolation Forest Anomaly Distribution" imageSrc="/static/results/anomaly_visual.png" />
            </div>
          </div>
        )}
      ...
      {/* PHISHGUARD AI ASSISTANT DRAWER */}
      {chatOpen && (
        <div className="fixed bottom-6 right-6 w-96 max-w-[calc(100vw-3rem)] h-[560px] cyber-panel flex flex-col z-50 shadow-2xl shadow-cyan-500/10 animate-in slide-in-from-bottom-5 duration-300 border border-cyan-500/30">
          ...
          <div className="p-4 border-b border-white/[0.08] bg-[#07090f] flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-400 font-mono">Real-Time Threat Intelligence</span>
              </div>
            </div>
            <button 
          ...
          <div className="flex-grow p-4 overflow-y-auto space-y-3 font-sans text-xs">
            {chatMessages.map((msg, idx) => (
              <div key={idx} className={`flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`p-3.5 rounded-2xl max-w-[90%] ${
                  msg.sender === 'user' 
            ...
            {chatLoading && (
              <div className="text-cyan-400 font-mono text-[11px] animate-pulse flex items-center gap-2">
                <RefreshCw className="w-3 h-3 animate-spin" /> Evaluating threat telemetry...
              </div>
            )}
          </div>
          ...
          <div className="px-3 py-2 border-t border-white/[0.06] bg-black/40 flex items-center gap-1.5 overflow-x-auto text-[11px] font-mono">
            ...
            <button 
              onClick={() => handleSendChat("Is this URL safe?")}
              className="px-2.5 py-1 rounded-full bg-white/[0.04] border border-white/[0.08] hover:border-cyan-400 text-slate-300 flex-shrink-0 transition-colors"
            >
              Is URL safe?
            </button>
...
function FeatureBox({ label, value, highlight = false }) {
  return (
    <div className={`p-3.5 rounded-xl border text-left ${highlight ? 'bg-rose-950/20 border-rose-500/40 text-rose-200' : 'bg-white/[0.02] border-white/[0.06] text-slate-300'}`}>
      <div className="text-[10px] text-slate-400 font-mono uppercase tracking-wider font-semibold">{label}</div>
      <div className="font-mono text-xs font-bold mt-1 text-slate-100">{value}</div>
    </div>
  );
}
...
function VisualPlotCard({ title, imageSrc }) {
  return (
    <div className="cyber-panel p-5 space-y-3">
      <h4 className="font-mono text-xs font-bold text-slate-200 tracking-wider uppercase">{title}</h4>
